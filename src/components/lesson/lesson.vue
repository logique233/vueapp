<template>
  <div id="app">
    <section class="sec sec_course">
      <div class="hd">
        <h2 class="hd_tt">课程中心</h2>
        <a
          href="http://bbs.itheima.com/thread-435922-1-1.html"
          target="_blank"
          class="a_default a_more"
        >查看更多开班信息>></a>
      </div>
      <div class="lay_conke lay_conke2 lay_python">
        <!-- 首页课程中心人工智能+Python -->
        <div class="div_zong bigger">
          <a class="course_a" href="http://python.itheima.com/" target="_blank">
            <h3>Python+人工智能</h3>
            <strong>
              Python+人工智能
              <br />——人工智能主流语言
            </strong>
            <p class="a_gd">
              <img src="images/course_logo/python_icon.png" alt="python图标" />
            </p>
          </a>
        </div>
        <ul class="icon_ul">
          <li class="a_gd">
            <a href="special/pythonzly/index.html" target="_blank">
              <img class="a_gd" src="images/kind11.jpg" alt="python介绍" />
              <p class="a_gd">学科介绍</p>
            </a>
          </li>
          <li class="a_gd">
            <a href="special/pythonzly/index.html" target="_blank">
              <img class="a_gd" src="images/kind22.jpg" alt="python就业前景" />
              <p class="a_gd">就业前景</p>
            </a>
          </li>
          <li class="a_gd">
            <a href="course/pythontext.html" target="_blank">
              <img class="a_gd" src="images/kind33.jpg" alt="python课程大纲" />
              <p class="a_gd">课程大纲</p>
            </a>
          </li>
          <li class="a_gd">
            <a href="teacher.html#apython" target="_blank">
              <img class="a_gd" src="images/kind44.jpg" alt="python师资介绍" />
              <p class="a_gd">师资介绍</p>
            </a>
          </li>
          <!--小横条-->
          <p class="clear ad_box">
            <a href="course/pythonhistory.html" target="_blank">
              <img src="images/newslistPIC/1562306722011_Python.jpg" alt="开启全民学python时代" />
            </a>
          </p>
        </ul>
        <!-- 首页课程中心人工智能+Python开班 -->
        <div class="kb_info">
          <div class="hd">
            <span>Python+人工智能</span>
            <dl class="select">
              <dt class>
                北京校区
                <i>&nbsp;</i>
              </dt>
              <dd style="display: none;">
                <ul>
                  <li>
                    <a href="javascript:;">
                      北京校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      深圳校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      上海校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      郑州校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      武汉校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      西安校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      广州校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      长沙校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      杭州校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      南京校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                  <li>
                    <a href="javascript:;">
                      成都校区
                      <i>&nbsp;</i>
                    </a>
                  </li>
                </ul>
              </dd>
            </dl>
          </div>
          <div class="bd">
            <ul class="infoList">
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-12
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-25
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-12-22
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-12-25
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-12-01
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-12-14
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2020-01-11
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2020-01-14
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-08-20
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-09-24
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-07
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-12-20
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-09-07
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-10-19
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-25
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2020-01-07
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-09-24
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-08
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-12-25
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-08-29
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-10-17
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-26
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2020-01-12
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-10-18
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-10-22
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-27
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-09-22
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-13
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-17
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-12-22
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-08-21
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-10-10
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-11-24
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-12-20
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-10-29
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-10-30
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-12-13
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2020-01-10
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-08-18
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-10-16
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-07
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  基础班
                  <em></em>2019-10-28
                  <em></em>
                  <strong>已开班</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a href="javascript:;" class="itcast_click" rel="nofollow">
                  就业班
                  <em></em>2019-11-18
                  <em></em>
                  <strong>报名中</strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
              <li>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
                <a target="_blank">
                  <em></em>
                  <em></em>
                  <strong></strong>
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>
<script>
//TODO
export default {};
</script>
<style scoped>
   
</style>